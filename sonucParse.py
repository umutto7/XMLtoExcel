...
def excel_writer():
    ...
    path = os.path.join(os.path.expanduser("~"), "Desktop", str(excel_name) + '.xlsx')
    excel_writer.writer = ExcelWriter(path)



def sonuc_parse(filelistsonuc):
    count_main = 0
    for filename in filelistsonuc:
        ...
        # -----> ExcelWriter for second and more
        else:
...
#sonuc_parse(filelistsonuc, writer)